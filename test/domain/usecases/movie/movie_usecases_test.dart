...
void main() {
  ...
  group('getPopularMovies', () {
    ...
    test(
      'should return [tMovieListingsEntity] when the call to repository is successful',
      () async {
        // arrange
        when(
          mockMovieRepository.getPopularMovies(page: tPage),
    ...
    test(
      'should return a [NetworkException] when the call to repository is unsuccessful',
      () async {
        // arrange
        when(mockMovieRepository.getPopularMovies(page: tPage)).thenAnswer(
  ...
  group('getTopRatedMovies', () {
    ...
    test(
      'should return [MovieListingsEntity] when the call to repository is successful',
      () async {
        // arrange
        when(
          mockMovieRepository.getTopRatedMovies(page: tPage),
        ).thenAnswer((_) async => Right(tMovieListingsEntity));
        // act
    ...
    test(
      ...
      () async {
        // arrange
        when(mockMovieRepository.getTopRatedMovies(page: tPage)).thenAnswer(
          (_) async => Left(NetworkException.fromDioError(dioException)),
        );
        // act
        final result = await movieUsecases.getTopRatedMovies(page: 1);
  ...
  group('getMovieCredits', () {
    ...
    test(
      'should return [MovieCreditEntity] when the call to repository is successful',
      () async {
        // arrange
        when(
          mockMovieRepository.getMovieCredits(movieId: tMovieId),
    ...
    test(
      ...
      () async {
        // arrange
        when(mockMovieRepository.getMovieCredits(movieId: tMovieId)).thenAnswer(
          (_) async => Left(NetworkException.fromDioError(dioException)),
        );
        // act
        final result = await movieUsecases.getMovieCredits(movieId: tMovieId);

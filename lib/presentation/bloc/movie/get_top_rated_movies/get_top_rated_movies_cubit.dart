...
class GetTopRatedMoviesCubit extends Cubit<GetTopRatedMoviesState> {
  GetTopRatedMoviesCubit(this._movieUsecases)
  ...
  Future<void> getTopRatedMovies() async {
    try {
      ...
      result.fold((error) => emit(GetTopRatedMoviesError(message: error.message)), (
        success,
      ) {
        ...
        _page++;
        _movieList.addAll(
          success.movies?.where(
                // If a movie already exists in the movie list, it will not be added again.
                (movie) => _movieList.contains(movie) == false,
              ) ??
              [],
        );

...
import '../../../presentation/bloc/movie/get_saved_movies/get_saved_movies_cubit.dart';
import '../../../presentation/bloc/movie/toggle_bookmark/toggle_bookmark_cubit.dart';
    ...
    return BlocBuilder<GetSavedMoviesCubit, GetSavedMoviesState>(
      builder: (_, getSavedMoviesState) {
        if (getSavedMoviesState is GetSavedMoviesLoaded) {
          bool isBookmarked =
              getSavedMoviesState.movies?.any(
                (element) => element.id == movieDetailEntity?.id,
              ) ??
              false;

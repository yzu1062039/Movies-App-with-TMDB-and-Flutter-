...
class SocialMediaBottomSheet extends StatelessWidget with BaseBottomSheetMixin {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create:
          (context) =>
              GetIt.I<GetActorSocialMediaCubit>()
                ..getActorSocialMedia(actorId: actorId ?? '0'),
          ...
          builder: (context, state) {
            if (state is GetActorSocialMediaError) {
              return RetryButton(
                ...
                retryAction:
                    () => context
                        .read<GetActorSocialMediaCubit>()
                        .getActorSocialMedia(actorId: actorId),
              );
            }

            if (state is! GetActorSocialMediaLoaded) {
            ...
            if (list.every((element) => element == null)) {
              return const Center(
                child: Text('Social media account not found'),
              );
            }
                ...
                Expanded(
                  child: Row(
                    children: [
                      if (list[0] != null)
                        Expanded(
                          child: SocialButton.instagram(id: state.instagramId),
                        ),
                      if (list[1] != null)
                        Expanded(
                          child: SocialButton.twitter(id: state.twitterId),
                        ),
                      if (list[2] != null)
                        Expanded(
                          child: SocialButton.facebook(id: state.facebookId),
                        ),
                      if (list[3] != null)
                        Expanded(
                          child: SocialButton.youtube(id: state.youtubeId),
                        ),
                      if (list[4] != null)
                        Expanded(child: SocialButton.imdb(id: state.imdbId)),
                      if (list[5] != null)
                        Expanded(
                          child: SocialButton.tiktok(id: state.tiktokId),
                        ),
                    ],
                  ),
                ),
